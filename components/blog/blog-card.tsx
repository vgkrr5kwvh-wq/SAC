...
export default function BlogCard({ post, hero = false }: { post: { slug: string; title: string; excerpt: string | null; content: string; coverImageUrl: string | null; featured: boolean; publishedAt: Date; categories: Array<{ name: string; slug: string }> }; hero?: boolean }) {
  return <article className={`blog-card cms-blog-card${post.featured ? " is-featured" : ""}${hero ? " is-hero" : ""}`}>
