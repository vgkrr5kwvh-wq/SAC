...
export default function UniversityManagementOverview({ data }: { data: UniversityManagementOverviewApiResult }) {
  ...
  return <div className="university-management-detail">
    ...
    <nav className="university-management-tabs" aria-label="University management sections"><span aria-current="page">Overview</span><Link href={`/admin/university-data/universities/${university.id}/programs`}><b>Programs ({statistics.totalPrograms})</b></Link><Link href={`/admin/university-data/universities/${university.id}/scholarships`}><b>Scholarships ({statistics.totalScholarships})</b></Link><Link href={`/admin/university-data/universities/${university.id}/requirements`}><b>Admission Requirements ({data.tabCounts.admissionRequirements})</b></Link><Link href={`/admin/university-data/universities/${university.id}/tuition`}><b>Tuition ({data.tabCounts.tuitionRecords})</b></Link>{tabs.slice(4).map(([tab, count]) => <span key={tab} aria-disabled="true"><b>{tab} ({count})</b><small>Coming soon</small></span>)}</nav>
