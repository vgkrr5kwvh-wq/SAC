...
export default async function UniversityReviewPage() {
  const records = await prisma.importRecord.findMany({
    where: { status: { in: ["STAGED", "MANUAL_REVIEW"] } },
    include: {
        ...
        {records.map((record) => {
          ...
          return <article key={record.id} className="university-review-card">
            ...
            {enrichment ? <section className="university-enrichment-review">
              ...
              <div className="university-claim-groups">
                {[...groupedClaims.entries()].map(([key, claims]) => {
                  ...
                  return <article key={key}>
                    <header><strong>{preferred.fieldName}</strong><span>{preferred.conflictStatus.replaceAll("_", " ")}</span></header>
                    ...
                    {preferred.conflictStatus !== "NONE" ? <p><b>Reason:</b> Higher-authority value retained for the same scope; review required.</p> : null}
                  </article>;
                })}
              </div>
            </section> : null}
            <details><summary>Extracted payload</summary><pre>{JSON.stringify(payload, null, 2)}</pre></details>
            <ReviewControls recordId={record.id} enrichment={enrichment} />
          </article>;
        })}
