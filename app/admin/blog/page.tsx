...
export default async function AdminBlogPage({ searchParams }: { searchParams: Promise<{ page?: string | string[]; search?: string | string[] }> }) {
  ...
  try {
    result = await prisma.$transaction([
      ...
      prisma.blogPost.findMany({
        ...
        take: blogPageSize,
        select: { id: true, title: true, slug: true, status: true, featured: true, publishedAt: true, updatedAt: true, createdBy: { select: { name: true, email: true } } },
      }),
    ]);
  } catch {
  ...
  return <div className="admin-enquiries-page admin-blog-page">
    <section className="admin-dashboard-heading"><div><span className="login-eyebrow">Website content</span><h1>Blog CMS</h1><p>Create and maintain public study-abroad guidance.</p></div><div className="admin-heading-actions"><Link className="button secondary" href="/admin/blog/categories">Categories</Link><Link className="button primary" href="/admin/blog/new">New post</Link></div></section>
