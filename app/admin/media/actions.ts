...
const genericState = (message: string): MediaFormState => ({ message, errors: {} });

export async function createMediaAction(_state: MediaFormState, formData: FormData): Promise<MediaFormState> {
  const session = await auth();
  if (!hasAuthenticatedMediaAdmin(session) || !session?.user?.id) return genericState("Unable to add media asset.");
  const file = formData.get("file");
  if (!(file instanceof File)) return { message: "Please select an image.", errors: { file: ["Choose an image to upload."] } };
  const fileError = validateImageFile(file);
  if (fileError) return { message: "Please correct the highlighted fields.", errors: { file: [fileError] } };
  ...
  if (!metadata.success) return { message: "Please correct the highlighted fields.", errors: metadata.error.flatten().fieldErrors };

  try {
    const buffer = Buffer.from(await file.arrayBuffer());
    const dimensions = readImageDimensions(buffer, file.type);
    if (!dimensions) return { message: "Please correct the highlighted fields.", errors: { file: ["The image is invalid, corrupt, or exceeds 8000 × 8000 pixels."] } };
    const checksum = createHash("sha256").update(buffer).digest("hex");
    const duplicate = await prisma.mediaAsset.findUnique({ where: { checksum }, select: { id: true } });
    if (duplicate) return { message: "This image is already in the Media Library.", errors: { file: ["Duplicate image."] } };
...
export async function updateMediaAction(id: string, _state: MediaFormState, formData: FormData): Promise<MediaFormState> {
  const session = await auth();
  if (!hasAuthenticatedMediaAdmin(session) || !session?.user?.id) return genericState("Unable to update media asset.");
  if (!isMediaAssetId(id)) return genericState("Unable to update media asset.");
  ...
  try {
    ...
    if (error && typeof error === "object" && "digest" in error) throw error;
    return genericState("Unable to update media asset.");
  }
}
